...
-(void) setup
{
    ...
    self.files = [manager contentsOfDirectoryAtPath:[directoryURL path] error:nil];
    
    if([self.files indexOfObject:@"My Vacation"] == NSNotFound)
    {
        [VacationHelper openVacation:@"My Vacation" usingBlock:^(UIManagedDocument *doc){}];
        self.files = [self.files arrayByAddingObject:@"My Vacation"];
    }
...
-(void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    [VacationHelper openVacation:[[sender textLabel] text] usingBlock:^(UIManagedDocument *doc){
        [segue.destinationViewController setPhotoDatabase:doc];
    }];
}
